...
function success(position) {
  ...
  latitude.textContent = position.coords.latitude + "°"; //2 levels deep in the object. (position, then coords)
  longitude.textContent = position.coords.longitude + "°";
  // STEP 3d: Build out the link to OpenStreetMap
  //First Step for integrating 3rd party APIs
...
if (!navigator.geolocation) {
  ...
  navigator.geolocation.getCurrentPosition(success, error); //arguments are for if the function is successful or not. If the user does not allow for location logging, then the error function will trigger
}
